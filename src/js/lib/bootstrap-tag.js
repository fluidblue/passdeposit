  ...
  , show: function () {
      ...
      that.input = $('<input type="text">')
        ...
        .on('keydown', function ( event ) {
          if ( event.keyCode == 188 || event.keyCode == 13 || event.keyCode == 9 || event.keyCode == 32 ) {
            if ( $.trim($(this).val()) && ( ( !that.element.siblings('.typeahead').length || that.element.siblings('.typeahead').is(':hidden') ) || event.keyCode == 32 || event.keyCode == 188 ) ) {
